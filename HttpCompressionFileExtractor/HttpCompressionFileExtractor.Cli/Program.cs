	...
	internal class Program {
		...
		static async Task Main (string[] args) {
			...
			var listCommand = new Command ("list", "查看HTTP压缩包文件列表") {
				urlOption, searchTextOption, isRegexOption, isWebApiOption, isIndentOption
			};
			rootCommand.AddCommand (listCommand);
			listCommand.SetHandler (ListAsync, urlOption, searchTextOption, isRegexOption, isWebApiOption, isIndentOption);
			var downloadCommand = new Command ("download", "下载HTTP压缩包内文件") {
				urlOption, pathOption, outputPathOption, isWebApiOption, isIndentOption
			};
			rootCommand.AddCommand (downloadCommand);
			downloadCommand.SetHandler (DownloadAsync, urlOption, pathOption, outputPathOption, isWebApiOption, isIndentOption);
			try {
		...
		static async Task ListAsync (string url, string searchText, bool isRegex, bool isWebApi, bool isIndent) {
			try {
				CommandLineWriter.IsWebApi = isWebApi;
				if (isIndent) {
					CommandLineWriter.Formatting = Formatting.Indented;
				}
				var fileName = Path.GetFileName (new Uri (url).LocalPath);
		...
		static async Task DownloadAsync (string url, string path, string outputPath, bool isWebApi, bool isIndent) {
			try {
				CommandLineWriter.IsWebApi = isWebApi;
				if (isIndent) {
					CommandLineWriter.Formatting = Formatting.Indented;
